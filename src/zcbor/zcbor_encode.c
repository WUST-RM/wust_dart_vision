...
static bool
encode_header_byte(zcbor_state_t* state, zcbor_major_type_t major_type, uint8_t additional) {
    ZCBOR_CHECK_ERROR();
    ZCBOR_CHECK_PAYLOAD();
...
static bool value_encode_len(
    ...
    if (!encode_header_byte(state, major_type, get_additional(result_len, u8_result[0]))) {
        ZCBOR_FAIL();
    }
...
static bool str_start_encode(
    zcbor_state_t* state,
    const struct zcbor_string* input,
    zcbor_major_type_t major_type
) {
    if (input->value
        && ((zcbor_header_len_ptr(&input->len, sizeof(input->len)) + input->len
             + (size_t)state->payload)
...
static bool
str_encode(zcbor_state_t* state, const struct zcbor_string* input, zcbor_major_type_t major_type) {
...
static bool
list_map_start_encode(zcbor_state_t* state, size_t max_num, zcbor_major_type_t major_type) {
...
static bool
list_map_end_encode(zcbor_state_t* state, size_t max_num, zcbor_major_type_t major_type) {
#ifdef ZCBOR_CANONICAL
    size_t list_count =
        ((major_type == ZCBOR_MAJOR_TYPE_LIST) ? state->elem_count : (state->elem_count / 2));
...
bool zcbor_float64_encode(zcbor_state_t* state, const double* input) {
    if (!value_encode_len(state, ZCBOR_MAJOR_TYPE_SIMPLE, input, sizeof(*input))) {
        ZCBOR_FAIL();
    }
...
bool zcbor_multi_encode_minmax(
    size_t min_encode,
    size_t max_encode,
