...
#define ZCBOR_VERSION_STR \
    ZCBOR_STRINGIFY(ZCBOR_VERSION_MAJOR) \
    "." ZCBOR_STRINGIFY(ZCBOR_VERSION_MINOR) "." ZCBOR_STRINGIFY(ZCBOR_VERSION_BUGFIX)

/** Monotonically increasing integer representing the version. */
#define ZCBOR_VERSION \
    ((ZCBOR_VERSION_MAJOR << 24) + (ZCBOR_VERSION_MINOR << 16) + (ZCBOR_VERSION_BUGFIX << 8))
...
#define ZCBOR_ENFORCE_CANONICAL(state) \
    (state->constant_state ? state->constant_state->enforce_canonical \
                           : ZCBOR_ENFORCE_CANONICAL_DEFAULT)

#define ZCBOR_MANUALLY_PROCESS_ELEM_DEFAULT false

#define ZCBOR_MANUALLY_PROCESS_ELEM(state) \
    (state->constant_state ? state->constant_state->manually_process_elem \
                           : ZCBOR_MANUALLY_PROCESS_ELEM_DEFAULT)
...
#define ZCBOR_VALUE_IS_INDEFINITE_LENGTH \
    31 ///! The list or map has indefinite length, and will instead be terminated by a 0xFF token.
...
#define ZCBOR_FLAG_RESTORE \
    1UL ///! Restore from the backup. Overwrite the current state with the state from the backup.
...
#define ZCBOR_FLAG_KEEP_DECODE_STATE \
    8UL ///! Keep the pre-restore decode state (everything only used for decoding)
...
#define ZCBOR_ERR_INVALID_VALUE_ENCODING \
    21 ///! When ZCBOR_CANONICAL is defined, and the incoming data is not encoded with minimal length, or uses indefinite length array.
 ...
 */
int zcbor_entry_function(
    const uint8_t* payload,
    size_t payload_len,
    void* result,
    size_t* payload_len_out,
    zcbor_state_t* state,
    zcbor_decoder_t func,
    size_t n_states,
    size_t elem_count
);
 ...
 */
bool zcbor_validate_string_fragments(struct zcbor_string_fragment* fragments, size_t num_fragments);
 ...
 */
bool zcbor_splice_string_fragments(
 ...
 */
bool zcbor_compare_strings(const struct zcbor_string* str1, const struct zcbor_string* str2);
