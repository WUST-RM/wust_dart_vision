...
bool zcbor_undefined_put(
    zcbor_state_t* state,
    const void* unused
); /* 'undefined' CBOR simple value */
...
bool zcbor_tag_encode(
    zcbor_state_t* state,
    uint32_t* tag
); /* CBOR tag. Note that zcbor_tag_encode()'s argument was changed to be a pointer. See also zcbor_tag_put(). */
...
bool zcbor_float16_bytes_encode(
    zcbor_state_t* state,
