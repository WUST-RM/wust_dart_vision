...
_Static_assert(
    (sizeof(zcbor_state_t) >= sizeof(struct zcbor_state_constant)),
...
bool zcbor_new_backup(zcbor_state_t* state, size_t new_elem_count) {
    ...
    if ((state->constant_state->current_backup) >= state->constant_state->num_backups) {
    ...
    memcpy(&state->constant_state->backup_list[i], state, sizeof(zcbor_state_t));
...
bool zcbor_process_backup(zcbor_state_t* state, uint32_t flags, size_t max_elem_count) {
    ...
    if (flags & ZCBOR_FLAG_RESTORE) {
        ...
        memcpy(state, &state->constant_state->backup_list[i], sizeof(zcbor_state_t));
    }
...
bool zcbor_validate_string_fragments(
...
bool zcbor_splice_string_fragments(
    ...
    for (size_t i = 0; i < num_fragments; i++) {
        if ((total_len > *result_len) || (fragments[i].fragment.len > (*result_len - total_len))) {
            return false;
        }
        memcpy(&result[total_len], fragments[i].fragment.value, fragments[i].fragment.len);
        total_len += fragments[i].fragment.len;
    }
...
bool zcbor_compare_strings(const struct zcbor_string* str1, const struct zcbor_string* str2) {
    return (str1 != NULL) && (str2 != NULL) && (str1->value != NULL) && (str2->value != NULL)
        && (str1->len == str2->len) && (memcmp(str1->value, str2->value, str1->len) == 0);
}
...
int zcbor_entry_function(
    ...
    if (payload_len_out != NULL) {
        *payload_len_out = MIN(payload_len, (size_t)state[0].payload - (size_t)payload);
    }
...
#define F16_MAX \
    65520 /* Lowest float32 value that rounds up to float16 infinity.
...
/* Rounding: */
#define SUBNORM_ROUND_MSK \
    (F32_MANTISSA_MSK | (1 << F32_EXPO_OFFS)) /* mantissa + lsb of expo for
								     * tiebreak. */
#define SUBNORM_ROUND_BIT_MSK (1 << (F32_EXPO_OFFS - 1)) /* msb of mantissa (0x400000) */
#define NORM_ROUND_MSK \
    (F32_MANTISSA_MSK >> (F16_EXPO_OFFS - 1)) /* excess mantissa when going from
    ...
    *(uint32_t*)&abs_input = value32 & ~(1 << F32_SIGN_OFFS);
    ...
    if (abs_input <= (F16_MIN / 2)) {
        ...
        uint16_t rounding_bit =
            ...
            ((mantissa & NORM_ROUND_BIT_MSK) && ((mantissa & NORM_ROUND_MSK) != NORM_ROUND_BIT_MSK)
            );
 ...
 */
__attribute__((__weak__)) size_t strnlen(const char* s, size_t maxlen) {
    size_t i;
