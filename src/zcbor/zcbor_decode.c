...
static bool value_extract(
    ...
    if ((additional == ZCBOR_VALUE_IS_INDEFINITE_LENGTH) && (indefinite_length_array != NULL)) {
        ...
        ZCBOR_ERR_IF(ZCBOR_ENFORCE_CANONICAL(state), ZCBOR_ERR_INVALID_VALUE_ENCODING);
        ...
        if (len == 0) {
            *result_offs = additional;
            ...
            if (ZCBOR_ENFORCE_CANONICAL(state) && !ZCBOR_IS_FLOAT(header_byte)) {
                ZCBOR_ERR_IF(
                    (zcbor_header_len_ptr(result, result_len) != (len + 1)),
...
bool zcbor_int_decode(zcbor_state_t* state, void* result, size_t result_size) {
    ...
    if (major_type != ZCBOR_MAJOR_TYPE_PINT && major_type != ZCBOR_MAJOR_TYPE_NINT) {
...
static bool str_overflow_check(zcbor_state_t* state, size_t len) {
    ...
    if (len > (size_t)(state->payload_end - state->payload)) {
        zcbor_log("error: 0x%zu > 0x%zu\r\n", len, (state->payload_end - state->payload));
        ERR_RESTORE(ZCBOR_ERR_NO_PAYLOAD);
    }
    return true;
}

static bool str_start_decode_with_overflow_check(
    zcbor_state_t* state,
    struct zcbor_string* result,
    zcbor_major_type_t exp_major_type
) {
...
static bool start_decode_fragment(
    zcbor_state_t* state,
...
bool zcbor_bstr_start_decode_fragment(zcbor_state_t* state, struct zcbor_string_fragment* result) {
    PRINT_FUNC();
...
static bool
str_decode(zcbor_state_t* state, struct zcbor_string* result, zcbor_major_type_t exp_major_type) {
...
static bool str_decode_fragment(
    zcbor_state_t* state,
    struct zcbor_string_fragment* result,
    zcbor_major_type_t exp_major_type
) {
...
static bool
str_expect(zcbor_state_t* state, struct zcbor_string* result, zcbor_major_type_t exp_major_type) {
...
static bool list_map_start_decode(zcbor_state_t* state, zcbor_major_type_t exp_major_type) {
    ...
    if (!zcbor_new_backup(state, indefinite_length_array ? ZCBOR_LARGE_ELEM_COUNT : new_elem_count))
    {
        FAIL_RESTORE();
    }
...
bool zcbor_array_at_end(zcbor_state_t* state) {
    ...
    return (
        (!indefinite_length_array && (state->elem_count == 0))
        || (indefinite_length_array && (state->payload < state->payload_end)
            && (*state->payload == 0xFF))
    );
}
...
static size_t zcbor_current_max_elem_count(zcbor_state_t* state) {
    return (
        state->decode_state.indefinite_length_array ? ZCBOR_LARGE_ELEM_COUNT
                                                    : state->decode_state.map_elem_count * 2
    );
}
...
__attribute__((used)) static size_t get_current_index(zcbor_state_t* state, uint32_t index_offset) {
...
static bool manipulate_flags(zcbor_state_t* state, uint32_t mode) {
    ...
    ZCBOR_ERR_IF((index >= state->decode_state.map_elem_count), ZCBOR_ERR_MAP_FLAGS_NOT_AVAILABLE);
...
static bool allocate_map_flags(zcbor_state_t* state, size_t old_flags) {
    ...
    if (extra_bytes) {
        ...
        memset(
            &state->decode_state.map_search_elem_state[new_bytes - extra_bytes],
            0xFF,
            extra_bytes
        );
    }
...
bool zcbor_unordered_map_search(
    zcbor_decoder_t key_decoder,
    zcbor_state_t* state,
    void* key_result
) {
    PRINT_FUNC();
...
bool zcbor_unordered_map_end_decode(zcbor_state_t* state) {
    ...
    ZCBOR_ERR_IF(
        !zcbor_array_at_end(state) && state->decode_state.counting_map_elems,
    ...
    if (state->decode_state.map_elem_count > 0) {
        ...
        for (size_t i = 0; i < zcbor_flags_to_bytes(state->decode_state.map_elem_count); i++) {
            if (state->decode_state.map_search_elem_state[i] != 0) {
                zcbor_log(
                    "unprocessed element(s) in map: [%zu] = 0x%02x\n",
                    i,
                    state->decode_state.map_search_elem_state[i]
                );
...
bool zcbor_any_skip(zcbor_state_t* state, void* result) {
    ...
    bool indefinite_length_array = false;
    bool* ila_ptr = ((major_type == ZCBOR_MAJOR_TYPE_MAP) || (major_type == ZCBOR_MAJOR_TYPE_LIST))
...
void zcbor_new_decode_state(
    zcbor_state_t* state_array,
    size_t n_states,
